package org.webdriver.mindaiappui.utils;
...
import org.webdriver.mindaiappui.config.Config;
	...
	private void outputResult(List<ITestResult> fullResults, String path){
		...
		try {
			...
			for (int i = 0; i < fullResults.size(); i++)
			{
				...
				if (!fullResults.get(i).getMethod().getDescription().isEmpty())
				{
					if (fullResults.get(i).getParameters().length>0)
					{
						...
						if (parameters[0].toString().contains("民")||parameters[0].toString().contains("贷")||parameters[0].toString().contains("天")
								||parameters[0].toString().contains("下")||parameters[0].toString().contains("@2017")) {
			...
			sb.append("</div>\n"
					...
					+ "<div id=\"footer\" >技术支持：©2016 民加科风信息技术有限公司 粤ICP备14079473号-2</div>");
			sb.append("</body>\n</html>\n");
			...
			sb2.append("<div id=\"logo\"><img src=\"http://oib9559ke.bkt.clouddn.com/QQ%E5%9B%BE%E7%89%8720170116191701.jpg\"></img></div>\n");
          ...
          /* sb2.append("</table>\n");*/
			...
			sb2.append("</table>\n</div>\n");
			sb2.append("</div>\n"
					+ "<div id=\"footer\" style=\"font-size:14px\" >技术支持：©2016 民加科风信息技术有限公司 粤ICP备14079473号-2</div>");
			sb2.append("</body>\n</html>\n");
			System.out.println("收件人地址："+Recipients);
